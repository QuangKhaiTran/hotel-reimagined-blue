        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
          {contactInfo.map((item, index) => (
            <motion.div
              ...
              whileHover={{ y: -8 }}
              className="bg-card p-8 rounded-3xl shadow-sm hover:shadow-md border border-border/50 text-center transition-all duration-300"
            >
              ...
              <span className="inline-block bg-secondary text-secondary-foreground px-4 py-1.5 rounded-full text-sm">
                {item.note}
              </span>
            </motion.div>
          ))}
